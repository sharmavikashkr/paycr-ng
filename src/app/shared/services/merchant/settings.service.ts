...
export class SettingsService {
  ...
  saveAccount(name: string, gstin: string): Observable<Merchant> {
    const body = new HttpParams()
      .set('name', name)
      .set('gstin', gstin);
    return this.httpClient.put<Merchant>(environment.APP_URL + '/merchant/account/update',
      body.toString(),
      {
  ...
  saveAddress(address: Address): Observable<Merchant> {
    return this.httpClient.put<Merchant>(environment.APP_URL + '/merchant/address/update', address);
  }
